...
async def get_posts(current_user: User = Depends(get_current_user)):  # Use Clerk dependency
    """
...
async def create_post(post: Post, current_user: User = Depends(get_current_user)):  # Use Clerk dependency
    """
    Requires a valid Clerk JWT.
    """
    supabase = get_supabase()
    post.id = str(uuid.uuid4())
    post.user_id = current_user.id # Get user ID from Clerk
    response = supabase.table(POSTS_TABLE).insert(post.dict()).execute()
    return response.data[0]

@app.get("/saved_posts", response_model=List[Post])
async def get_saved_posts(current_user: User = Depends(get_current_user)):  # Use Clerk dependency
    """
    Requires a valid Clerk JWT.
    """
    supabase = get_supabase()
    response = supabase.table(SAVED_POSTS_TABLE).select("post_id").eq("user_id", current_user.id).execute()
...
async def save_post(post_id: str, current_user: User = Depends(get_current_user)):  # Use Clerk dependency
    """
    Requires a valid Clerk JWT.
    """
    supabase = get_supabase()
    response = supabase.table(SAVED_POSTS_TABLE).insert({
        "user_id": current_user.id,  # Get user ID from Clerk
        "post_id": post_id
    }).execute()
    return {"message": f"Post {post_id} saved successfully"}

@app.delete("/saved_posts/{post_id}")
async def unsave_post(post_id: str, current_user: User = Depends(get_current_user)):  # Use Clerk dependency
    """
    Requires a valid Clerk JWT.
    """
    supabase = get_supabase()
    response = supabase.table(SAVED_POSTS_TABLE).delete().eq("user_id", current_user.id).eq("post_id", post_id).execute()
...
async def get_following(current_user: User = Depends(get_current_user)):  # Use Clerk dependency
    """
    Requires a valid Clerk JWT.
    """
    supabase = get_supabase()
    response = supabase.table(FOLLOWS_TABLE).select("followed_id").eq("follower_id", current_user.id).execute()
...
async def follow_profile(profile_id: str, current_user: User = Depends(get_current_user)):  # Use Clerk dependency
    """
    Requires a valid Clerk JWT.
    """
    supabase = get_supabase()
    response = supabase.table(FOLLOWS_TABLE).insert({
        "follower_id": current_user.id,  # Get user ID from Clerk
        "followed_id": profile_id
    }).execute()
    return {"message": f"Profile {profile_id} followed successfully"}

@app.post("/profiles/unfollow/{profile_id}")
async def unfollow_profile(profile_id: str, current_user: User = Depends(get_current_user)):  # Use Clerk dependency
    """
    Requires a valid Clerk JWT.
    """
    supabase = get_supabase()
    response = supabase.table(FOLLOWS_TABLE).delete().eq("follower_id", current_user.id).eq("followed_id", profile_id).execute()
    return {"message": f"Profile {profile_id} unfollowed successfully"}

@app.post("/blacklist/{profile_id}")
async def blacklist_profile(profile_id: str, current_user: User = Depends(get_current_user)):  # Use Clerk dependency
    """
    Requires a valid Clerk JWT.
    """
    supabase = get_supabase()
...
async def remove_from_blacklist(profile_id: str, current_user: User = Depends(get_current_user)):  # Use Clerk dependency
    """
    Requires a valid Clerk JWT.
    """
    supabase = get_supabase()
    response = supabase.table(BLACKLIST_TABLE).delete().eq("user_id", current_user.id).eq("blocked_id", profile_id).execute()
    return {"message": f"Profile {profile_id} removed from blacklist successfully"}

@app.post("/interactions", response_model=Interaction)
async def create_interaction(interaction: Interaction, current_user: User = Depends(get_current_user)):  # Use Clerk dependency
    """
    Requires a valid Clerk JWT.
    """
    supabase = get_supabase()
    interaction.id = str(uuid.uuid4())
    interaction.user_id = current_user.id  # Get user ID from Clerk
...
async def search_posts(query: str, current_user: User = Depends(get_current_user)):  # Use Clerk dependency
    """
    Requires a valid Clerk JWT.
    """
    supabase = get_supabase()
